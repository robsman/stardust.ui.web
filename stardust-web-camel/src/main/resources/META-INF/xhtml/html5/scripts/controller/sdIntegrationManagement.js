 ...
 *******************************************************************************/
 (function() {
	'use strict';
 angular.module("camel-ui")
 .controller('sdIntegrationManagement', ['$scope', '$http', '$q', '$timeout','$parse', 'sdUtilService',  'sdViewUtilService', 'eventBus', 'sdI18nService',function($scope, $http, $q, $timeout,$parse, sdUtilService, sdViewUtilService, eventBus, sdI18nService)
{
 //app.
	...
	function ajaxCamelContext(params, restUrl) {
		...
		var sortedCamelContexts = $scope.sortOn( data, params );
			...
			if (sortedCamelContexts.length > 0) {
				for (var count = fromCamelItem; count < toCamelItem; count++) {
		...
		httpResponse
				.success(
						function(data) {
							...
							if (sortedCamelContexts.length > 0) {
								for (var count = fromProducerRouteItem; count < toProducerRouteItem; count++) {
									if (sortedCamelContexts.length > count) {
