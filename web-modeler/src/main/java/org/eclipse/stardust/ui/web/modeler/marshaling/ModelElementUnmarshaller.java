...
public class ModelElementUnmarshaller {
	...
	public ModelElementUnmarshaller() {
		...
		symbolPropertiesMap.put(ActivitySymbolType.class, new String[] { "x",
				"y" });
		modelElementPropertiesMap.put(ActivitySymbolType.class, new String[] {
				"name", "description" });

		symbolPropertiesMap.put(StartEventSymbol.class,
				new String[] { "x", "y" });
		modelElementPropertiesMap.put(StartEventSymbol.class, new String[] {
				"name", "description" });

		symbolPropertiesMap
				.put(EndEventSymbol.class, new String[] { "x", "y" });
	...
	public void populateFromJson(IModelElement element, JsonObject json) {
	   System.out.println("Unmarshalling: " + element + " " + json);
		...
		if (element instanceof ProcessDefinitionType) {
			...
			if (((ActivitySymbolType) element).getActivity().getName()
					.toLowerCase().startsWith("gateway")) {
	...
	private void updateActivitySymbol(ActivitySymbolType activitySymbol,
		...
		if (ModelerConstants.MANUAL_ACTIVITY.equals(extractString(activityJson,
		...
		} else if (ModelerConstants.SUBPROCESS_ACTIVITY.equals(extractString(
			...
			BpmApplicationActivityBuilder applicationActivity = newApplicationActivity(ModelUtils
					.findContainingProcess(activity));
			applicationActivity.setActivity(activity);
			ModelType applicationModel = ModelUtils
					.findContainingModel(application);
	...
	private void updateGatewaySymbol(ActivitySymbolType activitySymbol,
		...
		ActivityType activity = activitySymbol.getActivity();
		JsonObject activityJson = gatewaySymbolJson
				.getAsJsonObject(ModelerConstants.MODEL_ELEMENT_PROPERTY);
	 ...
	 */
	private void mapDeclaredSymbolProperties(IModelElement symbol,
			JsonObject symbolJson, String[] symbolProperties) {
		if (symbol != null) {
