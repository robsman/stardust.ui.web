		...
		function(m_utils, m_constants, m_commandsController, m_command, m_user) {
			...
			function Session(sessionOwner) {
				...
				Session.prototype.processCommand = function(command) {
					...
					if (command.type == m_constants.ACCEPT_INVITE_COMMAND && joined) {
						var oldObject = {"account" : command.oldObject.sessionOwner};
						...
						var oldObject = {"account" : command.account};
						...
						console.log("Session starts working on "+ m_constants.UPDATE_INVITED_USERS_COMMAND);
						this.owner.color = command.ownerColor;
							...
							if(command.operation == "updateProspects"){
								...
								jQuery.each(command.oldObject.users, function(item, value){
									sessionCallbackObj.prospects.push(m_user.createUser(value.account, value.firstName, value.lastName, value.email, null));		
								});
							}else if(command.operation == "updateCollaborators"){
								sessionCallbackObj.participants = [];
								jQuery.each(command.oldObject.users, function(item, value){
									var user = m_user.createUser(value.account, value.firstName, value.lastName, value.email, null, value.color);
									var idx = -1;
									...
									if(idx != -1){
										sessionCallbackObj.prospects.splice(idx, 1);
									}
									sessionCallbackObj.participants.push(user);
