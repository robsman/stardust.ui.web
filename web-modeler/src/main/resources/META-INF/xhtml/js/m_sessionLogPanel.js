...
define(
		[ "m_utils", "m_constants", "m_user", "m_session", "m_command",
				...
				"m_communicationController", "m_websocketModel", "m_websocketInvite", "mustache" ],
		function(m_utils, m_constants, m_user, m_session, m_command,
			...
			function SessionLogPanel() {
				...
				this.chatTextArea.keyup({ panel : this},
								function(event) {
									var code = (event.keyCode ? event.keyCode
											: event.which);
									if (code == 13 && m_session.getInstance().joined) {
										m_websocketModel.send(jQuery.stringifyJSON(m_command.createSubmitChatMessageCommand(jQuery("#sessionLogPanel #chatTextArea").val())));
										jQuery("#sessionLogPanel #chatTextArea").val("");
										jQuery("#sessionLogPanel #chatTextArea").focus();
									} else if ((!m_session.getInstance().joined)
											&& code == 13) {
										m_commandsController.broadcastCommand(m_command.createSubmitChatMessageCommand("You are currently not in a chat/model session, invite people to create a modelsession. "));
										jQuery("#sessionLogPanel #chatTextArea").val("");
										jQuery("#sessionLogPanel #chatTextArea").focus();
									}
								});
				...
				SessionLogPanel.prototype.processCommand = function(command) {
					...
					var view = null;
					var template = "<img src=\"{{{image}}}\"/> </td><td valign=\"top\"><span id=\"userTag\"> {{account}} </span>" +
					...
					"<table><tr><td valign=\"top\" align=\"left\">";
						...
						if (command.type == m_constants.REQUEST_JOIN_COMMAND) {
							...
							if (command.account != user.account) {
								...
								"Wainting for the invited user to accept or decline your offer. If the user is not online this could take a while. "; 
							...
							template += "The joining of User {{firstname}} {{lastname}} was confirmed. </td></tr></table></td></tr>"; 
							...
							console.log("SessionLogPanel starts working on "+ m_constants.UPDATE_INVITED_USERS_COMMAND);
							...
							var ownerList = jQuery("<li><span>"+owner.account+" (owner) </span></li>");
							...
							jQuery.each(participants, function(item, value) {
										var tableRow = jQuery("<li><span>"+ value.account + "</span></li>")
							...
							jQuery.each(prospects, function(item, value) {
								var tableRow = jQuery("<li><span>"+ value.account + " (Prospect)</span></li>")
								participantRow.append(tableRow);
							
							});
							
							console.log("SessionLogPanel stops working on "+ m_constants.UPDATE_INVITED_USERS_COMMAND);
