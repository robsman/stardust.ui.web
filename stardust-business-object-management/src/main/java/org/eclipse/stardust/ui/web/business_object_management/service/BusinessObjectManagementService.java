...
public class BusinessObjectManagementService {
	...
	private JsonObject getFundTestJson() {
		...
		fundFieldsJson.add(toTestField("name", "Fund Name", "string", true,
				false));
	...
	private JsonObject toTestField(String id, String name, String type,
		...
		return toTestField(id, name, type, key, primaryKey, "0-1");
	}
	...
	private JsonObject toTestToManyField(String id, String name, String type,
			boolean key, boolean primaryKey) {
		return toTestField(id, name, type, key, primaryKey, "0-N");
	}
	 ...
	 */
	private JsonObject toTestField(String id, String name, String type,
			boolean key, boolean primaryKey, String cardinality) {
	 ...
	 * query.setPolicy(new
	 * BusinessObjectQuery.Policy(BusinessObjectQuery.Option.WITH_DESCRIPTION));
	 * 
	 * Map<Long, JsonObject> modelsMap = CollectionUtils.newMap();
	 * 
	 * BusinessObjects bos = getQueryService().getBusinessObjects(query); for
	 * (BusinessObject bo : bos) { JsonArray businessObjectsJson = null;
	 * JsonObject modelJson = modelsMap.get(bo.getModelOid()); if (modelJson ==
	 * null) { modelJson = new JsonObject(); modelJson.addProperty("oid",
	 * bo.getModelOid()); businessObjectsJson = new JsonArray();
	 * modelJson.add("businessObjects", businessObjectsJson);
	 * modelsJson.add(modelJson); } else { businessObjectsJson =
	 * modelJson.getAsJsonArray("businessObjects"); }
	...
	/*
	 * private JsonObject toJson(BusinessObject bo) { JsonObject json = new
	 * JsonObject(); json.addProperty("id", bo.getId());
	 * json.addProperty("name", bo.getName()); json.add("fields",
	 * toJson(bo.getItems())); return json; }
	 */

	/*
	 * private JsonArray toJson(List<Definition> items) { JsonArray json = new
	 * JsonArray(); if (items != null) { for (Definition definition : items) {
	 * json.add(toJson((Definition) definition)); } } return json; }
	 */
	...
	private void addStructuralInformation(JsonObject json,
			List<TypedXPath> xPaths, JsonObject types) {
	...
	/*
	 * private JsonObject toJson(Definition definition) { JsonObject json = new
	 * JsonObject(); json.addProperty("id", definition.getName());
	 * json.addProperty("name", definition.getName()); json.addProperty("type",
	 * "string"); // TODO json.addProperty("key", definition.isKey());
	 * json.addProperty("primaryKey", definition.isPrimaryKey());
	 * json.add("fields", toJson(definition.getItems())); return json; }
	 */
	...
	public JsonObject getBusinessObjectInstances(String modelOid,
		...
		if (businessObjectId.equals("Member")) {
			businessObjectInstances.add(getMemberInstance("4711", "Haile",
					"Selassie", "1", "Scheme-1", "SA"));
			businessObjectInstances.add(getMemberInstance("0815", "Jan",
					"Smuts", "2", "Scheme-2", "SA"));
			 ...
			 * (Long.parseLong(modelOid), businessObjectId); query.setPolicy(new
			 * BusinessObjectQuery
			 * .Policy(BusinessObjectQuery.Option.WITH_VALUES)); BusinessObjects
			 * bos = getQueryService().getBusinessObjects(query);
			 * 
			 * for (BusinessObject bo : bos) { List<Value> values =
			 * bo.getValues(); if (values != null) { for (Value value : values)
	...
	/*
	 * private JsonElement toJson(Value value) { Serializable object =
	 * value.getValue(); if (object instanceof Map) { return
	 * toMapValueJson((Map<?, ?>) object); } return new JsonObject(); }
	 */
