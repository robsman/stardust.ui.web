		...
		for (DeployedModel deployedModel : modelCache.getAllModels()) {
			if (!PredefinedConstants.PREDEFINED_MODEL_ID.equals(deployedModel
				...
				for (Data data : allData) {
					if (data.getModelOID() == deployedModel.getModelOID()) {
				...
				if (xPath.getType() == BigData.NULL || xPath.isEnumeration()) {
					...
					if (!types.has(key)) {
						...
						if (xPath.isEnumeration()) {
							type.add("values", listToJsonArray(xPath
									.getEnumerationValues()));
						} else {
							List<TypedXPath> childXPaths = xPath
									.getChildXPaths();
							if (childXPaths != null && !childXPaths.isEmpty()) {
								addStructuralInformation(type, childXPaths,
										types, null);
							}
						}
						types.add(key, type);
