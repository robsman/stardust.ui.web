...
(function () {
    if (Raphael.vml) {
        ...
        Raphael.el.desaturate = function (enable) {
			if (this.node != null) {
				...
				if (enable != "0") {
					s.filter = f + " progid:DXImageTransform.Microsoft.BasicImage(grayScale=1)";
				} else {
					s.filter = f;
				}
			}
        ...
        Raphael.el.desaturate = function (enable) {
            if (enable != "0") {
				...
				//$(colorMatrix, {type: "saturate"});
				
				$(colorMatrix, {values: "0.3 0.59 0.11 0 0 \
					 0.3 0.59 0.11 0 0 \
					 0.3 0.59 0.11 0 0 \
					 0      0      0      1 0"});
