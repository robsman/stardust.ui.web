	...
	function NameFilterDirective(sdUtilService, $filter) {
		return {
			...
			template : '<form name="filterForm">'
					...
					+ '<tr>'
					+ '<td><label class="label-item">{{i18n(\'views-common-messages.common-filterPopup-userName-firstName-label\')}}</label></td>'
					+ '<td>'
					+ '<input type="text" ng-model="filterData.firstName" id="firstName" name="firstName" ng-model-onblur> </input>'
					+ '</td>'
					+ '</tr>'
					+ '<tr>'
					+ '<td><label class="label-item">{{i18n(\'views-common-messages.common-filterPopup-userName-lastName-label\')}}</label></td>'
					+ '<td>'
