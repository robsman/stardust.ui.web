/***********************************************************************************************************************
 * Copyright (c) 2016 SunGard CSA LLC and others. All rights reserved. This program and the accompanying materials are
 * made available under the terms of the Eclipse Public License v1.0 which accompanies this distribution, and is
 * available at http://www.eclipse.org/legal/epl-v10.html Contributors: SunGard CSA LLC - initial API and implementation
 * and/or initial documentation
 **********************************************************************************************************************/
(function () {
	'use strict';

	angular.module('bcc-ui').directive('sdProcessDescriptor',
			['sdUtilService', 'sdProcessDescriptorService', '$q', 'sdI18nService', processDescriptor]);
	function processDescriptor (sdUtilService, sdProcessDescriptorService, $q, sdI18nService) {
		return {
			...
			templateUrl : sdUtilService.getBaseUrl()
					+ 'plugins/html5-business-control-center/scripts/directives/partials/processDescriptor.html',
		...
		function DescriptorTableCompiler (scope, element, attr, ctrl) {
			...
			DescriptorTableCompiler.prototype.fetchData = function () {
				...
				sdProcessDescriptorService
						.getProcessDescriptors(this.poid)
						.then(
								function (descriptorList) {
									for ( var i in descriptorList) {
										if (descriptorList[i].type == "TimeStamp"
												|| (descriptorList[i].type == "Calender"
														&& descriptorList[i].hideTime != true && descriptorList[i].editable)) {
											descriptorList[i].value = new Date(descriptorList[i].value).getTime();
										} else if (descriptorList[i].type == "Calender"
												&& descriptorList[i].hideTime == true) {
									...
									self.updateMsg = response.data.message;
								});
