...
var DocumentUtils = function() {
  ...
  this.ippWindow = window;

  return {
    openWindow: function(contextPath) {
      openWindow(contextPath);
    },
    closeWindow: function(str, subStr) {
      closeWindow(str, subStr);
    },
  ...
  function openWindow(contextPath) {
    ...
    var completeUrl = window.location.protocol + "//" + window.location.host + contextPath;

    this.popupWindow = window.open(completeUrl, 'ippExternalDocumentWindow', parameters);
  ...
  function closeWindow() {
    if (!this.popupWindow) {
      ...
      if (this.ippWindow && this.ippWindow.popupWindow) {
        this.ippWindow.popupWindow.close();
      }
    } else {
      if (false == this.popupWindow.closed) {
  ...
  function setCookie(c_name, value, expiredays) {
    ...
    myDocument = this.popupWindow.opener.document;
  ...
  function getMessage(messageProp, defaultMsg) {
    if (InfinityBPMI18N && InfinityBPMI18N.common) {
      ...
      if (propVal && propVal != "") { return propVal; }
    }
